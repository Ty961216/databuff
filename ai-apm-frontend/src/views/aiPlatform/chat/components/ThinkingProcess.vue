...
export default class ThinkingProcess extends Vue {
  ...
  private toggleExpand () {
    ...
    this.$nextTick(() => {
      ...
      // 展开后触发 mermaid 重试；collapse 过渡期间高度可能为 0，分多次派发
      if (this.expanded) {
        this.retryMermaidInThinking();
        window.setTimeout(() => this.retryMermaidInThinking(), 120);
        window.setTimeout(() => this.retryMermaidInThinking(), 360);
      }
    });
  }

  private retryMermaidInThinking () {
